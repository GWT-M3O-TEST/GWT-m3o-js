...
consume(request: ConsumeRequest): Promise<m3o.Stream<ConsumeRequest, ConsumeResponse>> {
		return this.client.stream("event", "Consume", request);
	};
	// Publish a event to the event stream.
publish(request: PublishRequest): Promise<PublishResponse> {
		return this.client.call("event", "Publish", request) as Promise<PublishResponse>;
	};
	// Read stored events
